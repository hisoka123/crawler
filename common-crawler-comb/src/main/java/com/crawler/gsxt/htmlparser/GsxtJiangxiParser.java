...
@Component
public class GsxtJiangxiParser extends AbstractGsxtParser {

private static final Log LOOGER = LogFactory.getLog(GsxtJiangxiParser.class);

	public AicFeedJson jiangxiResultParser(String html,Boolean isDebug)  {

		LOOGER.info("The method of GsxtJiangxiParser.jiangxiResultParser is begin !");
		...
		Document gsgsxx_djxx = Jsoup.parseBodyFragment(htmlMap.get("gsgsxx_djxx_jbxx").toString());
		// 工商公示信息->登记信息->基本信息
		Element baseinfo = gsgsxx_djxx.getElementById("jbxx");
		if (null == baseinfo) {
			baseinfo = gsgsxx_djxx.getElementById("main");
		}
		...
		for (int i = 1; i < ths.size(); i++) {
			...
			if (ths.get(i).text().trim().contains("组成形式")) {
				gsgsDjJbInfo.setFormType(baseinfo.select("tbody").select("tr")
						.select("td").get(i - 1).text());// 登记状态
			}
		}
		...
		Document gsgsxx_djxx_gdxx = Jsoup.parseBodyFragment(htmlMap.get("gsgsxx_djxx_tzrxx").toString());
		Element touziren_tbody = gsgsxx_djxx_gdxx.getElementById("table3");			
		if (null != touziren_tbody) {		
			Elements touziren_trs = touziren_tbody.select("tr");
			int touziren_nums = touziren_trs.size();			
			if (touziren_nums > 3) {
				for (int i = 2; i < touziren_nums-2; i++) {
					// 股东信息
		...
		if (gsgsxx_djxx_tzrxx_xqs_size > 0) {
			for (Map<String, Object> gsgsxx_djxx_tzrxx_xq : gsgsxx_djxx_tzrxx_xqs) {
				...
				Elements gsgsxx_djxx_tzrxx_xq_trs = gsgsxx_djxx_tzrxx_xq_page.getElementById("sifapanding").select("tbody").select("tr");
		...
		Document gsgsxx_djxx_gdbg = Jsoup.parseBodyFragment(htmlMap.get("gsgsxx_djxx_bgxx").toString());
		Element biangeng = gsgsxx_djxx_gdbg.getElementById("table3").select("tbody").get(1);
		if (null != biangeng) {
			Elements biangeng_trs = biangeng.select("tr");
			int biangeng_trs_size = biangeng_trs.size();
			if (biangeng_trs_size > 2) {
				for (int i = 1; i < biangeng_trs_size-2; i++) {
		...
		if (null != htmlMap.get("gsgsxx_baxx")
			...
			Element beian_zyryxx = gsgsxx_baxx.getElementById("main");			
			if (null !=beian_zyryxx) {				
			Elements beian_zyryxx_tables=beian_zyryxx.select("table");
			int table_size=beian_zyryxx_tables.size();
			for (int k = 0; k< table_size; k++) {
				Element  beian_zyryxx_table=beian_zyryxx_tables.get(k);
				Elements beian_zyryxx_ths=getElements(beian_zyryxx_table, "th");
				Elements beian_zyryxx_trs=getElements(beian_zyryxx_table, "tr");
				Elements beian_zyryxx_tds=getElements(beian_zyryxx_table, "td");
				String table_name=beian_zyryxx_ths.get(0).text().trim();				
				if (table_name.contains("主要人员信息")) {
					 gsgsBaZyryInfos = new ArrayList<AicpubArchivePrimemberInfo>();
				   for (int t = 0; t < beian_zyryxx_ths.size(); t++) {
				...
				if (table_name.contains("参加经营的家庭成员姓名")) {
			        gsgsBaZyryInfos = new ArrayList<AicpubArchivePrimemberInfo>();					
					  for (int t = 0; t < beian_zyryxx_ths.size(); t++) {
						   int beian_zyryxx_size = beian_zyryxx_trs.size();
							if (beian_zyryxx_size > 2) {
								for (int i = 2; i < beian_zyryxx_size-1; i++) {								
									String name1 = beian_zyryxx_trs.get(i).select("td").get(1).text();								
									String name2 = beian_zyryxx_trs.get(i).select("td").get(3).text();
									// 主要人员信息
									AicpubArchivePrimemberInfo gsgsBaZyryInfo1 = new AicpubArchivePrimemberInfo();
									// 主要人员信息
									AicpubArchivePrimemberInfo gsgsBaZyryInfo2 = new AicpubArchivePrimemberInfo();
			...
			if (table_name.contains("主管部门（出资人）信息") ) {
					aicpubArchiveMainDeptInfos= new ArrayList<AicpubArchiveMainDeptInfo>();							
						Elements zyryTrElements = beian_zyryxx_trs;
				...
				if (table_name.contains("清算信息")) {
					AicpubArchiveClearInfo gsgsBaQsInfo = new AicpubArchiveClearInfo();
					Elements beian_qsxx_trs=beian_zyryxx_trs;
					if (isDebug) {
						gsgsBaQsInfo.setHtml(beian_qsxx_trs.toString());
					}
		...
		if (null != htmlMap.get("gsgsxx_baxx2")
				&& !StringUtils.isEmpty(htmlMap.get("gsgsxx_baxx2").toString())) {
			Document gsgsxx_baxx = Jsoup.parseBodyFragment(htmlMap.get(
					"gsgsxx_baxx2").toString());
			Element gsgsxx_baxx_el = gsgsxx_baxx.getElementById("main");	
			if (null !=gsgsxx_baxx_el ) {
				Element gsgsxx_baxx_table=gsgsxx_baxx_el.getElementById("table3");
				if (null != gsgsxx_baxx_table) {
					Elements gsgsxx_baxx_tbody=gsgsxx_baxx_table.select("tbody");
					Elements beian_baxx_ths=getElements(gsgsxx_baxx_table, "th");
					String table_name=beian_baxx_ths.get(0).text().trim();
					if (table_name.contains("主管部门（出资人）信息")) {
						if (gsgsxx_baxx_tbody.size()>=2) {
							Elements gsgsxx_baxx_trs=gsgsxx_baxx_tbody.get(1).select("tr");
							if (gsgsxx_baxx_trs.size()>2) {
								for (int i = 1; i < gsgsxx_baxx_trs.size()-1; i++) {
									Elements gsgsxx_baxx_tds = gsgsxx_baxx_trs.select("td");
									String type = gsgsxx_baxx_tds.get(0).text();
									String name = gsgsxx_baxx_tds.get(1).text();
									String idType = gsgsxx_baxx_tds.get(2).text();
									String idNum = gsgsxx_baxx_tds.get(3).text();						
		...
		if (null != htmlMap.get("gsgsxx_dcdydjxx")
			...
			Element dongchandiya_et = gsgsxx_dcdydjxx_dcdydjxx_page.getElementById("main");			
		...
		if (null != htmlMap.get("gsgsxx_gqczdjxx")
			...
			// 工商公示信息->股权出资登记信息
			Element guquanchuzhi_et = gsgsxx_gqczdjxx_gqczdjxx_page.getElementById("main");
...
//						gsgsGqczdjGqczdjInfo.setChangeSitu(guquanchuzhi_trs.get(i).select("td").get(10).text());// 变化情况
		...
		if (null != htmlMap.get("gsgsxx_xzcfxx")
			...
			Element xingzhengchufa = gsgsxx_xzcfxx_xzcfxx.getElementById("main");
			if (null != xingzhengchufa) {
				Elements xingzhengchufa_trs = xingzhengchufa.select("tbody").select("tr");
		...
		if (null != htmlMap.get("gsgsxx_jyycxx")
			...
			Element yichangminglu = gsgsxx_jyycxx_jyycxx
					.getElementById("main");
			Elements yichangminglu_trs = yichangminglu.select("tbody").select("tr");
		...
		if (null != htmlMap.get("gsgsxx_yzwfxx")
			...
			Element heimingdan_et = gsgsxx_yzwfxx_yzwfxx_page.getElementById("main");			
			if (heimingdan_et!=null) {
				Elements yanzhongwf_trs = heimingdan_et.select("tbody").select("tr");				
				int yanzhongwf_trs_size=yanzhongwf_trs.size();
				if (yanzhongwf_trs_size>2) {
						for (int i = 2; i < yanzhongwf_trs_size; i++) {	
							Elements yanzhongwf_tds= yanzhongwf_trs.get(i).select("td");
							String lryzwfqymdCause = yanzhongwf_tds.get(1).text();
							String lrDate = yanzhongwf_tds.get(2).text();
							String ycyzwfqymdCause = yanzhongwf_tds.get(3).text();
							String ycDate = yanzhongwf_tds.get(4).text();
							String zcjdAuthority = yanzhongwf_tds.get(5).text();
		...
		if (null != htmlMap.get("gsgsxx_ccjcxx")
			...
			Elements chouchaxinxi_trs = gsgsxx_ccjcxx_ccjcxx
					.getElementById("main").getElementById("tableChoucha").select("tr");
					...
					for (int k = 0; k < table_size; k++) {						
						  ...
						  if (k==0) {
							  ...
							  //	企业年报信息--> 网站或网店信息
									...
									int wzwdxxTrs_size=qygsxx_qynb_info_trs.size();				
									if (wzwdxxTrs_size>2) {
										for (int i = 2; i < wzwdxxTrs_size; i++) {					
											EntpubAnnreportWebsiteInfo qygsQynbWzhwdInfo=new EntpubAnnreportWebsiteInfo();
											qygsQynbWzhwdInfo.setType(qygsxx_qynb_info_trs.get(i).select("td").get(0).text());
											qygsQynbWzhwdInfo.setName(qygsxx_qynb_info_trs.get(i).select("td").get(1).text());
											qygsQynbWzhwdInfo.setWebsite(qygsxx_qynb_info_trs.get(i).select("td").get(2).text());
								...
								List<EntpubAnnreportStohrinvestInfo> qygsQynbGdjczInfos = new ArrayList<EntpubAnnreportStohrinvestInfo>();// 股东及出资信息									
									int qyjbxx_gd_trs_size = qygsxx_qynb_info_trs.size();
									if (qyjbxx_gd_trs_size > 2) {
										for (int i = 2; i < qyjbxx_gd_trs_size; i++) {
											...
											qygsQynbGdjczInfo.setStockholder(qygsxx_qynb_info_trs
													.get(i).select("td").get(0).text());// 股东（发起人）
											qygsQynbGdjczInfo.setSubAmount(qygsxx_qynb_info_trs
													.get(i).select("td").get(1).text());// 认缴出资额（万元）
											qygsQynbGdjczInfo.setSubDateTime(qygsxx_qynb_info_trs.get(i)
													.select("td").get(2).text());// 认缴出资时间
											qygsQynbGdjczInfo.setSubMethod(qygsxx_qynb_info_trs
													.get(i).select("td").get(3).text());// 认缴出资方式
											qygsQynbGdjczInfo.setPaidAmount(qygsxx_qynb_info_trs
													.get(i).select("td").get(4).text());// 实缴出资额（万元）
											qygsQynbGdjczInfo.setPaidDateTime(qygsxx_qynb_info_trs.get(i)
													.select("td").get(5).text());// 实缴出资时间
											qygsQynbGdjczInfo.setPaidMethod(qygsxx_qynb_info_trs
								...
								List<EntpubAnnreportExtinvestInfo> qygsQynbDwtzInfos = new ArrayList<EntpubAnnreportExtinvestInfo>();// 对外投资信息								
									int dwtzxx_gd_trs_size = qygsxx_qynb_info_trs.size();
									if (dwtzxx_gd_trs_size > 2) {
										for (int i = 2; i < dwtzxx_gd_trs_size; i++) {
											...
											qygsQynbDwtzInfo.setEnterpriseName(qygsxx_qynb_info_trs.get(i).select("td").get(0).text());//投资设立企业或购买股权企业名称
											qygsQynbDwtzInfo.setRegNum(qygsxx_qynb_info_trs.get(i).select("td").get(1).text());// 股东（发起人）
							...
							if (qygsxx_qynb_info_trs.size()>2) {
								for (int i = 2;i < qygsxx_qynb_info_trs.size(); i++) {
									EntpubAnnreportExtguaranteeInfo qygsQynbDwtgbzdbInfo = new EntpubAnnreportExtguaranteeInfo();
									qygsQynbDwtgbzdbInfo.setCreditor(qygsxx_qynb_info_trs.get(i).select("td").get(0).text());
									qygsQynbDwtgbzdbInfo.setDebtor(qygsxx_qynb_info_trs.get(i).select("td").get(1).text());
									
									qygsQynbDwtgbzdbInfo.setPriCredRightType(qygsxx_qynb_info_trs.get(i).select("td").get(2).text());
									qygsQynbDwtgbzdbInfo.setPriCredRightAmount(qygsxx_qynb_info_trs.get(i).select("td").get(3).text());
									qygsQynbDwtgbzdbInfo.setExeDebtDeadline(qygsxx_qynb_info_trs.get(i).select("td").get(4).text());
									qygsQynbDwtgbzdbInfo.setGuaranteePeriod(qygsxx_qynb_info_trs.get(i).select("td").get(5).text());
									qygsQynbDwtgbzdbInfo.setGuaranteeMethod(qygsxx_qynb_info_trs.get(i).select("td").get(6).text());
									qygsQynbDwtgbzdbInfo.setGuaranteeScope(qygsxx_qynb_info_trs.get(i).select("td").get(7).text());
						...
						}else if (table_name.contains("股权变更记录")) {						
						 //股权变更记录   
						List<EntpubAnnreportEquchangeInfo> qygsQynbGqbgInfos = new ArrayList<EntpubAnnreportEquchangeInfo>();
			    			if (qygsxx_qynb_info_trs.size()>2) {					
								for (int i = 2; i < qygsxx_qynb_info_trs.size(); i++) {
									...
									qygsQynbGqbgInfo.setStockholder(qygsxx_qynb_info_trs.get(i).select("td").get(0).text()); //股东（发起人）							
									qygsQynbGqbgInfo.setPreOwnershipRatio(qygsxx_qynb_info_trs.get(i).select("td").get(2).text());//变更前股权比例
									qygsQynbGqbgInfo.setPostOwnershipRatio(qygsxx_qynb_info_trs.get(i).select("td").get(3).text());	//变更后股权比例
									qygsQynbGqbgInfo.setDateTime(qygsxx_qynb_info_trs.get(i).select("td").get(4).text());//股权变更日期							
						...
						List<EntpubAnnreportModifyInfo> qygsQynbXgjlInfos = new ArrayList<EntpubAnnreportModifyInfo>();									
							if (qygsxx_qynb_info_trs.size()>2) {					
								for (int i = 2; i < qygsxx_qynb_info_trs.size(); i++) {
									...
									qygsQynbXgjlInfo.setItem(qygsxx_qynb_info_trs.get(i).select("td").get(1).text());
									qygsQynbXgjlInfo.setPreContent(qygsxx_qynb_info_trs.get(i).select("td").get(2).text());
									qygsQynbXgjlInfo.setPostContent(qygsxx_qynb_info_trs.get(i).select("td").get(3).text());
									qygsQynbXgjlInfo.setDateTime(qygsxx_qynb_info_trs.get(i).select("td").get(4).text());							
		...
		if (null != htmlMap.get("qygsxx_gdjczxx")
			...
			Element touziren_el=qygsxx_gdjczxx.getElementById("touziren");
			if (null !=touziren_el ) {
				Element touziren_table=touziren_el.select("table").get(0);				
				Elements gdjczxx_trs = touziren_table.select("tbody").select("tr");		
				int gdjczxx_trs_size = gdjczxx_trs.size();
				if (gdjczxx_trs_size > 3) {
					for (int j = 3; j < gdjczxx_trs_size; j +=3) {						
						// 企业公示信息->股东及出资信息
						EntpubSStohrinvestInfo qygsGdjczGdjczInfo = new EntpubSStohrinvestInfo();
						qygsGdjczGdjczInfo.setStockholder(gdjczxx_trs.get(j).select("td").get(0).text());// 股东
						qygsGdjczGdjczInfo.setSubAmount(gdjczxx_trs.get(j).select("td").get(1).text());// 认缴额（万元）
						qygsGdjczGdjczInfo.setPaidAmount(gdjczxx_trs.get(j).select("td").get(2).text());// 实缴额（万元）
						EntpubSStohrinvestInfo.Detail rjDetail = qygsGdjczGdjczInfo.new Detail();// 认缴明细
						rjDetail.amount = gdjczxx_trs.get(j).select("td").get(4).text();// 出资额（万元）
						rjDetail.method = gdjczxx_trs.get(j).select("td").get(5).text();// 出资方式
						rjDetail.dateTime = gdjczxx_trs.get(j).select("td").get(6).text();// 出资日期
						EntpubSStohrinvestInfo.Detail sjDetail = qygsGdjczGdjczInfo.new Detail();// 实缴明细				
						sjDetail.amount = gdjczxx_trs.get(j).select("td").get(8).text();// 出资额（万元）
						sjDetail.method = gdjczxx_trs.get(j).select("td").get(9).text();// 出资方式
						sjDetail.dateTime = gdjczxx_trs.get(j).select("td").get(10).text();// 出资日期
						sjDetail.showDate=gdjczxx_trs.get(j).select("td").get(11).text();
				...
				Element bgxx_tb=touziren_el.select("table").last();//变更信息				
				if (null !=bgxx_tb) {
					Elements bgxx_trs=getElements(bgxx_tb,"tr");
		...
		if (null != htmlMap.get("qygsxx_gqbgxx")
			...
			Element guquanbiangeng_tb = qygsxx_gqbgxx_page.getElementById("gudongguquan");			
			if (null !=guquanbiangeng_tb) {
				Elements guquanbiangeng_trs=getElements(guquanbiangeng_tb,"tr");
		...
		if(null != htmlMap.get("qygsxx_xzxkxx") && !StringUtils.isEmpty(htmlMap.get(
			...
			Element xingzhengxuke_tb = qygsxx_xzxkxx_page.getElementById("xingzhengxuke");
			if (null !=xingzhengxuke_tb) {
				Elements xingzhengxuke_trs=getElements(xingzhengxuke_tb,"tr");			
		...
		if (null != htmlMap.get("qygsxx_zscqczdjxx")
			...
			Element zhishichanquan_tbs = qygsxx_zscqczdjxx_page.getElementById("zhishichanquan");			
			if (null !=zhishichanquan_tbs) {
				Elements zhishichanquan_trs=getElements(zhishichanquan_tbs,"tr");
				...
				if (zhishichanquan_tr_size>2) {				
					for (int i = 2; i < zhishichanquan_tr_size; i++) {
						...
						qygsZscqczdjZscqczdjInfo.setRegNum(zhishichanquan_trs.get(i).select("td").get(1).text());// 注册号
						qygsZscqczdjZscqczdjInfo.setName(zhishichanquan_trs.get(i).select("td").get(2).text());// 名称
						qygsZscqczdjZscqczdjInfo.setType(zhishichanquan_trs.get(i).select("td").get(3).text());// 种类
						qygsZscqczdjZscqczdjInfo.setMortgagorName(zhishichanquan_trs.get(i).select("td").get(4).text());// 出质人名称
						qygsZscqczdjZscqczdjInfo.setMortgageeName(zhishichanquan_trs.get(i).select("td").get(5).text());// 质权人名称
						qygsZscqczdjZscqczdjInfo.setPledgeRegDeadline(zhishichanquan_trs.get(i).select("td").get(6).text());// 质权登记期限
						qygsZscqczdjZscqczdjInfo.setChangeSitu(zhishichanquan_trs.get(i).select("td").get(7).text());// 变化情况
						qygsZscqczdjZscqczdjInfo.setStatus(zhishichanquan_trs.get(i).select("td").get(8).text());// 状态（暂无）
		...
		if (null != htmlMap.get("qygsxx_xzcfxx")
			...
			Element xingzhengchufa_et = qygsxx_xzcfxx_page.getElementById("xingzhengchufa");
			Elements xingzhengchufa_trs=getElements(xingzhengchufa_et,"tr");
			if (xingzhengchufa_trs.size()>2) {
				for (int i = 2; i < xingzhengchufa_trs.size(); i++) {
					...
					qygsXzcfXzcfInfo.setPunishRepNum(xingzhengchufa_trs.get(i).select("td").get(1).text());// 行政处罚决定书文号
					qygsXzcfXzcfInfo.setIllegalActType(xingzhengchufa_trs.get(i).select("td").get(2).text());// 违法行为类型
					qygsXzcfXzcfInfo.setPunishContent(xingzhengchufa_trs.get(i).select("td").get(3).text());// 行政处罚内容
					qygsXzcfXzcfInfo.setDeciAuthority(xingzhengchufa_trs.get(i).select("td").get(4).text());// 作出行政处罚决定机关名称
					qygsXzcfXzcfInfo.setDeciDateTime(xingzhengchufa_trs.get(i).select("td").get(5).text());// 作出行政处罚决定日期 
					qygsXzcfXzcfInfo.setShowDate(xingzhengchufa_trs.get(i).select("td").get(6).text());//公示日期
					qygsXzcfXzcfInfo.setNote(xingzhengchufa_trs.get(i).select("td").get(7).text());// 备注（暂无）
		...
		if (null != htmlMap.get("qtbmgsxx_xzxkxx")
			...
			// 其他部门公示信息->行政许可信息列表
			Element xingzhengxuke_table = qtbmgsxx_xzxkxx_page.getElementById("xingzhengxuke").select("table").first();			
			if (null !=xingzhengxuke_table) {
				Elements xingzhengxuke_tb_trs=getElements(xingzhengxuke_table,"tr");
				int xingzhengxuke_tb_tr_size=xingzhengxuke_tb_trs.size();
				if (xingzhengxuke_tb_tr_size>2) {
					for (int i = 2; i < xingzhengxuke_tb_tr_size; i++) {
					...
					if (isDebug) {
						qtbmgsXzxkInfo.setHtml(xingzhengxuke_table.toString());
					}				
				}
			}
		...
		if (null != htmlMap.get("qtbmgsxx_xzcfxx")
			...
			Element  xingzhengchufa_tb=qtbmgsxx_xzcfxx_page.getElementById("xingzhengchufa").select("table").first();			
			if (null !=xingzhengchufa_tb) {
				Elements xingzhengchufa_trs=getElements(xingzhengchufa_tb,"tr");			
				if (xingzhengchufa_trs.size()>2) {				
					for (int i = 2; i < xingzhengchufa_trs.size(); i++) {
		...
		if (null != htmlMap.get("sfxzgsxx_gqdjxx")
			...
			Element guquandongjie_tb = sfxzgsxx_gqdjxx_list_page.getElementById("main").select("table").first();
			if (null != guquandongjie_tb) {				
				Elements guquandongjie_trs=getElements(guquandongjie_tb,"tr");
				if (guquandongjie_trs.size()>2) {
					...
					if (isDebug) {
						sfxzgsGqdjInfo.setHtml(guquandongjie_tb.html());
					}					
				}
			}
		...
		if (null != htmlMap.get("sfxzgsxx_gqbgxx")
			...
			// 司法协助公示信息->股东变更信息
			Element guquanbiangeng_tb = sfxzgsxx_gqbgxx_list_page
					.getElementById("main").select("table").first();
			if (null != guquanbiangeng_tb) {		
				Elements  guquanbiangeng_trs=getElements(guquanbiangeng_tb,"tr");
				if (guquanbiangeng_trs.size()>2) {				
					...
					if (isDebug) {
					    sfxzgsGdbgInfo.setHtml(guquanbiangeng_tb.html());
					}					
				}
			}
