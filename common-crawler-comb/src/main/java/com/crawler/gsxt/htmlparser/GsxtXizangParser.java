...
@Component
public class GsxtXizangParser extends AbstractGsxtParser {

	public  AicFeedJson xizangResultParser(String html, Boolean isDebug) {  
		...
		for (int i = 1; i < jbxx_ths.size(); i++) {
			...
			if (jbxx_ths.get(i).text().trim().contains("营业场所")
	...
	//-----------------工商公示信息-->股权出质登记信息 start-----------------------
		...
		AicpubAdmpunishInfo gsgsXzcfInfo = new AicpubAdmpunishInfo();
			...
			//股东及出资信息-->股东及出资信息	
			List<EntpubSStohrinvestInfo> stohrInvestInfos=null;
			if (qygsxxDoc.getElementById("touziren")!=null) {
				stohrInvestInfos=new ArrayList<EntpubSStohrinvestInfo>();
				Element qygsgdczxxDiv = qygsgdczDiv.getElementById("gdDiv");
				Elements gdxxDetailTrs = qygsgdczxxDiv.select("tr");	
				
				if (null != qygsgdczxxDiv) {				
					if (gdxxDetailTrs.size()>4) {						
						for (int i = 3; i < gdxxDetailTrs.size(); i++) {
							EntpubSStohrinvestInfo stohrInvestInfo = new EntpubSStohrinvestInfo();
							Elements gdxxDetail_tds = gdxxDetailTrs.get(i).select("td");
							int tdSize = gdxxDetail_tds.size();
							if (tdSize == 3) {
								String stockholder = gdxxDetail_tds.get(0).text();
								String subAmount = gdxxDetail_tds.get(1).text();
								String paidAmount = gdxxDetail_tds.get(2).text();						
								stohrInvestInfo.setStockholder(stockholder);
								stohrInvestInfo.setSubAmount(subAmount);
								stohrInvestInfo.setPaidAmount(paidAmount);
							} else if (tdSize == 6) {
								...
								EntpubSStohrinvestInfo.Detail subDetail= stohrInvestInfo.new Detail();
								EntpubSStohrinvestInfo.Detail paidDetail = stohrInvestInfo.new Detail();
								...
								subDetail.method = sub_method;
								subDetail.amount = sub_amount;
								subDetail.dateTime = sub_czDate;
								paidDetail.method = paid_method;
								paidDetail.amount = paid_amount;
								paidDetail.dateTime = paid_czDate;
								
								subDetails.add(subDetail);
								paidDetails.add(paidDetail);
								stohrInvestInfo.setSubDetails(subDetails);
								stohrInvestInfo.setPaidDetails(paidDetails);
							} else if (tdSize == 9) {
								String stockholder = gdxxDetail_tds.get(0).text();
								String subAmount = gdxxDetail_tds.get(1).text();
								String paidAmount = gdxxDetail_tds.get(2).text();
								...
								String sub_amount = gdxxDetail_tds.get(4).text();
								String sub_czDate = gdxxDetail_tds.get(5).text();
								String sub_showDate = gdxxDetail_tds.get(6).text();
								String paid_method = gdxxDetail_tds.get(7).text();
								String paid_amount = gdxxDetail_tds.get(8).text();
								String paid_czDate = gdxxDetail_tds.get(9).text();
								String paid_showDate = gdxxDetail_tds.get(10).text();
								...
								subDetail.amount = sub_amount;
								subDetail.dateTime = sub_czDate;
								subDetail.showDate = sub_showDate;
								paidDetail.method = paid_method;
								paidDetail.amount = paid_amount;
								paidDetail.dateTime = paid_czDate;
								paidDetail.showDate=paid_showDate;
								stohrInvestInfo.setStockholder(stockholder);
								stohrInvestInfo.setSubAmount(subAmount);
								stohrInvestInfo.setPaidAmount(paidAmount);
								subDetails.add(subDetail);
							...
							stohrInvestInfos.add(stohrInvestInfo);
						}
				...
				qygsGdjczInfo.setStohrInvestInfos(stohrInvestInfos);	
