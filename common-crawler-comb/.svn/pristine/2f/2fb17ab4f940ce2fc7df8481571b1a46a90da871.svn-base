...
@Component
public class GsxtSichuanParser extends AbstractGsxtParser {

private static final Log LOOGER = LogFactory.getLog(GsxtSichuanParser.class);

	public AicFeedJson sichuanResultParser(String html,Boolean isDebug)  {
...
//				gsgsBaQsInfo.setLeader(beian_qingsuanrenyuan.get(1).text());// 清算组负责人
//				gsgsBaQsInfo.setMembers(null);// 清算组成员
//				gsgsBaInfo.setGsgsBaQsInfo(gsgsBaQsInfo);// 清算信息
				if (isDebug) {
				...
				int xingzhengxuke_trs_size = xingzhengxuke_trs.size();
					if (xingzhengxuke_trs_size > 2) {
						for (int i = 2; i < xingzhengxuke_trs_size-1; i++) {
							...
							qygsXzxkXzxkInfo.setPubDate(xingzhengxuke_trs.get(i)
			...
			if (null != gsgsxx_xzxkxx_xzxkxx_xqs
				...
				for (Map<String, Object> gsgsxx_xzxkxx_xzxkxx_xq : gsgsxx_xzxkxx_xzxkxx_xqs) {
					...
					if (null != gsgsxx_xzxkxx_xzxkxx_table) {
						...
						Elements gsgsxx_xzxkxx_xzxkxx_xq_trs=gsgsxx_xzxkxx_xzxkxx_table.select("tr");
						...
						if (bgxq_trs_size>3) {
							for (int i = 2; i < bgxq_trs_size-1; i++) {
								...
								admlicDetail.changeItem = gsgsxx_xzxkxx_xzxkxx_xq_trs.get(i).select("td").get(1).text();// 名称
								admlicDetail.changeDateTime = gsgsxx_xzxkxx_xzxkxx_xq_trs.get(i).select("td").get(2).text();// 注册号
								admlicDetail.changePreContent = gsgsxx_xzxkxx_xzxkxx_xq_trs.get(i).select("td").get(3).text();// 法定代表人（负责人）姓名
								admlicDetail.changePostContent = gsgsxx_xzxkxx_xzxkxx_xq_trs.get(i).select("td").get(4).text();// 违法行为类型
