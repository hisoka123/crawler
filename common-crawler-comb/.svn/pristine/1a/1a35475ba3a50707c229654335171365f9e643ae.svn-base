...
@Component
public class GsxtShaanxiParser {

	public AicFeedJson shaanxiResultParser(String resultHtmls, Boolean isDebug) {
		//解析result
		Gson gson = new Gson();
		...
		Object gsgsxx_djxx_object = resultHtmlMap.get("gsgsxx_djxx");
		if (gsgsxx_djxx_object != null) {
			//1.基本信息
			AicpubRegBaseInfo gsgsDjJbInfo = new AicpubRegBaseInfo();
			String gsgsxxDjHtml = (String)gsgsxx_djxx_object;
			Document gsgsxxDjDoc = Jsoup.parse(gsgsxxDjHtml);
			
			Element gsgsDjJbxxDiv = gsgsxxDjDoc.getElementById("jibenxinxi");
			if (gsgsDjJbxxDiv != null) {
				Elements gsgsDjJbxxTables = gsgsDjJbxxDiv.select("table");
				if (gsgsDjJbxxTables != null && !gsgsDjJbxxTables.isEmpty()) {
					Element gsgsDjJbxxTable = gsgsDjJbxxTables.get(0);
					Elements gsgsxxDjjbThs = gsgsDjJbxxTable.select("th");
					Elements gsgsxxDjjbTds = gsgsDjJbxxTable.select("td");
			...
			Object gsgsxx_djxx_tzrxx_xq_object = resultHtmlMap.get("gsgsxx_djxx_gdxx");
			...
			Element gsgsxxDjGdTable = gsgsxxDjDoc.getElementById("table_fr");
			...
			Element bgxx_table = gsgsxxDjDoc.getElementById("table_bg");
			if (bgxx_table != null) {
			...
			Object target_item_info_object = resultHtmlMap.get("target_item_info");
			if (target_item_info_object != null) {
				AicpubRegBaseInfo gsgsDjJbInfo = new AicpubRegBaseInfo();
				String targetItemInfoHtml = (String)target_item_info_object;
				Document targetItemInfoDoc = Jsoup.parse(targetItemInfoHtml);
				Elements ulElements = targetItemInfoDoc.select("ul");
				if (ulElements != null && !ulElements.isEmpty()) {
					Element ulElement = ulElements.get(0);
					if (ulElement != null) {
						Elements liElements = ulElement.select("li");
						if (liElements != null && !liElements.isEmpty()) {
							String entName = liElements.get(0).text();
							String[] split = entName.split(" ");
							if (split.length > 0) {
								gsgsDjJbInfo.setName(split[0]);
							}
							if (split.length > 1) {
								gsgsDjJbInfo.setRegStatus(split[1].replace("(", "").replace(")", ""));
							}
							
							if (liElements.size() > 1) {
								Element liElement = liElements.get(1);
								Elements spanElements = liElement.select("span");
								if (spanElements != null && !spanElements.isEmpty()) {
									int spanSize = spanElements.size();
									String num = spanElements.get(0).text();
									gsgsDjJbInfo.setNum(num);
									
									if (spanSize > 1) {
										String legalRepr = spanElements.get(1).text();
										gsgsDjJbInfo.setLegalRepr(legalRepr);
									}
									
									if (spanSize > 2) {
										String regAuthority = spanElements.get(2).text();
										gsgsDjJbInfo.setRegAuthority(regAuthority);
									}
									
									if (spanSize > 3) {
										String regDateTime = spanElements.get(3).text();
										gsgsDjJbInfo.setRegDateTime(regDateTime);
									}
								}
							}
						}
		...
		Object gsgs_baxx_page_object = resultHtmlMap.get("gsgs_baxx_page");
		if (gsgs_baxx_page_object != null) {
			String gsgsBaxxHtml = (String)gsgs_baxx_page_object;
			Document gsgsBaxxDoc = Jsoup.parse(gsgsBaxxHtml);
			
			//1.主要人员信息
			Element gsgsBaZyryTable = gsgsBaxxDoc.getElementById("table_ry1");
			if (gsgsBaZyryTable != null) {
				Elements gsgsBaZyryTrs = gsgsBaZyryTable.select("tr");
				for (Element zyryTr : gsgsBaZyryTrs) {
					if (!"".equals(zyryTr.attr("id"))) {
			...
			Element fzjgxxTable = gsgsBaxxDoc.getElementById("table_fr2");
			if (fzjgxxTable != null) {
				Elements gsgsBaFzjgTrs = fzjgxxTable.select("tr");
			...
			Element qsxxTable = gsgsBaxxDoc.getElementById("t32");
			if (qsxxTable != null) {
				Elements gsgsBaQsTrs = qsxxTable.select("tr");
			...
			Element gsgsBaZgbmTable = gsgsBaxxDoc.getElementById("table_fr");
		...
		Object gsgsxx_dcdydjxx_dcdydjxx_object = resultHtmlMap.get("gsgs_dcdydjxx_page");
		if (gsgsxx_dcdydjxx_dcdydjxx_object != null) {
			...
			Element gsgsDcdydjTable = gsgsDcdydjDoc.getElementById("table_dc");
			if (gsgsDcdydjTable != null) {
				...
				for (Element gsgsDcdydjTr : gsgsDcdydjTrs) {
					...
					if (gsgsDcdydjTds.size() == 7) {
						...
						String detail = gsgsDcdydjTds.get(6).text();
		...
		Object gsgsxx_gqczdjxx_gqczdjxx_object = resultHtmlMap.get("gsgs_gqczdjxx_page");
		if (gsgsxx_gqczdjxx_gqczdjxx_object != null) {
			...
			Element gsgsGqczdjTable = gsgsGqczdjDoc.getElementById("table_gq");
		...
		Object gsgsxx_xzcfxx_xzcfxx_object = resultHtmlMap.get("gsgs_xzcfxx_page");
		if (gsgsxx_xzcfxx_xzcfxx_object != null) {
			...
			Element gsgsXzcfTable = gsgsXzcfDoc.getElementById("table_gscfxx");
			if (gsgsXzcfTable != null) {
		...
		Object gsgsxx_jyycxx_jyycxx_object = resultHtmlMap.get("gsgs_jyycxx_page");
		if (gsgsxx_jyycxx_jyycxx_object != null) {
			...
			Element gsgsJyycTable = gsgsJyycDoc.getElementById("table_yc");
		...
		Object gsgsxx_yzwfxx_yzwfxx_object = resultHtmlMap.get("gsgs_yzwfxx_page");
		if (gsgsxx_yzwfxx_yzwfxx_object != null) {
			...
			Element gsgsYzwfTable = gsgsYzwfDoc.getElementById("table_wfxx");
		...
		Object gsgsxx_ccjcxx_ccjcxx_object = resultHtmlMap.get("gsgs_ccjcxx_page");
		if (gsgsxx_ccjcxx_ccjcxx_object != null) {
			...
			Element gsgsCcjcTable = gsgsCcjcDoc.getElementById("table_ccjc");
		...
		//-----------------企业公示信息-->企业年报 start-----------------------
		//企业年报列表信息--报送年度和发布日期
		Object qygsxx_qynb_object = resultHtmlMap.get("qygs_qynbxx_page");
		Object qygsxx_qynb_detail_object = resultHtmlMap.get("qygs_qynbxx_detail");
		if (qygsxx_qynb_object != null && qygsxx_qynb_detail_object != null) {
			List<String> qynbDetailList = (ArrayList<String>)qygsxx_qynb_detail_object;
			String qygsxx_qynb_page = (String)qygsxx_qynb_object;
			Document qygsxxDoc = Jsoup.parse(qygsxx_qynb_page);
			List<EntpubAnnreportInfo> qygsQynbInfos = new ArrayList<EntpubAnnreportInfo>();
			Element qygsnbTable = qygsxxDoc.getElementById("t30");
			if (qygsnbTable != null) {
				Elements qygsnbTrs = qygsnbTable.select("tr");
				int k = 0;
				for (int j = 2; j < qygsnbTrs.size(); j++) {
					...
					for (Element qygsnbxxTable : qygsnbxxTables) {
						...
						if ("企业基本信息".equals(itemTitle)) {
							...
							for (int i = 0; i < qyjbxxThs.size(); i++) {
								...
								switch (qyjbxxTh) {
									...
									case "从业人数":
										...
										break;
									case "隶属关系":
										qygsQynbJbInfo.setAffiliation(qyjbxxTd);
										break;
									default:
										break;
								}
						...
						} else if ("企业资产状况信息".equals(itemTitle) || "生产经营情况".equals(itemTitle)) {
							...
							for (Element wzwdxxTr : wzwdxxTrs) {
								if(!"".equals(wzwdxxTr.attr("id")) && "".equals(wzwdxxTr.attr("style"))) {
							...
							for (Element gdczxxTr : gdczxxTrs) {
								if(!"".equals(gdczxxTr.attr("id")) && "".equals(gdczxxTr.attr("style"))) {
							...
							for (Element dwtzxxTr : dwtzxxTrs) {
								if(!"".equals(dwtzxxTr.attr("id")) && "".equals(dwtzxxTr.attr("style"))) {
							...
							for (Element dwdbxxTr : dwdbxxTrs) {
								if(!"".equals(dwdbxxTr.attr("id")) && "".equals(dwdbxxTr.attr("style"))) {
							...
							for (Element gqbgxxTr : gqbgxxTrs) {
								if(!"".equals(gqbgxxTr.attr("id")) && "".equals(gqbgxxTr.attr("style"))) {
							...
							for (Element xgjlxxTr : xgjlxxTrs) {
								if(!"".equals(xgjlxxTr.attr("id")) && "".equals(xgjlxxTr.attr("style"))) {
		...
		Object qygs_gdjczxx_object = resultHtmlMap.get("qygs_gdjczxx_page");
		
		if (qygs_gdjczxx_object != null) {
			//股东及出资信息-->股东及出资信息
			List<EntpubSStohrinvestInfo> qygsGdjczGdjczInfos = new ArrayList<EntpubSStohrinvestInfo>();
			String qygs_gdjczxx_page = (String)qygs_gdjczxx_object;
			Document qygsgdjczDoc = Jsoup.parse(qygs_gdjczxx_page);
			Element qygsgdczxxDiv = qygsgdjczDoc.getElementById("tzrxx");
			if (qygsgdczxxDiv != null) {
				Element gdczxxTable = qygsgdczxxDiv.getElementById("table_qytzr");
				if (gdczxxTable != null) {
					Elements qygsxxGdjczTrs = gdczxxTable.select("tr");
			...
			Element qygsbgxxDiv = qygsgdjczDoc.getElementById("sifapanding");
			if (qygsbgxxDiv != null) {
				Element qygsbgxxTable = qygsbgxxDiv.getElementById("table_tzrxxbg");
				if (qygsbgxxTable != null) {
					Elements qygsbgxxTrs = qygsbgxxTable.select("tr");
					qygsbgxxTrs.remove(0);
					qygsbgxxTrs.remove(0);
					for (Element qygsbgxxTr : qygsbgxxTrs) {
						Elements qygsbgxxTds = qygsbgxxTr.select("td");
		...
		Object qygs_gqbgxx_object = resultHtmlMap.get("qygs_gqbgxx_page");
		if (qygs_gqbgxx_object != null) {
			String qygs_gqbgxx_page = (String)qygs_gqbgxx_object;
			Document qygsGqbgDoc = Jsoup.parse(qygs_gqbgxx_page);
			Element qygsGqbgxxDiv = qygsGqbgDoc.getElementById("sifapanding");
			if (qygsGqbgxxDiv != null) {
				Element qygsGqbgxxTable = qygsGqbgxxDiv.getElementById("table_tzrbgxx");
				if (qygsGqbgxxTable != null) {
					Elements qygsGqbgxxTrs = qygsGqbgxxTable.select("tr");
		...
		Object qygs_xzxkxx_object = resultHtmlMap.get("qygs_xzxkxx_page");
		if (qygs_xzxkxx_object != null) {
			String qygs_xzxkxx_page = (String)qygs_xzxkxx_object;
			Document qygsXzxxDoc = Jsoup.parse(qygs_xzxkxx_page);
			Element qygsXzxkxxDiv = qygsXzxxDoc.getElementById("xzxk");
			if (qygsXzxkxxDiv != null) {
				Element qygsXzxkxxTable = qygsXzxkxxDiv.getElementById("table_xzxk");
				...
				if (isDebug) {
					qygsXzxkInfo.setHtml(qygsXzxkxxDiv.toString());
				}
			}
		}
		...
		Object qygs_zscqczdjxx_object = resultHtmlMap.get("qygs_zscqczdjxx_page");
		if (qygs_zscqczdjxx_object != null) {
			String qygs_zscqczdjxx_page = (String)qygs_zscqczdjxx_object;
			Document qygsZscqczdjDoc = Jsoup.parse(qygs_zscqczdjxx_page);
			Element qygsZscqczdjxxDiv = qygsZscqczdjDoc.getElementById("sifapanding");
			if (qygsZscqczdjxxDiv != null) {
				Element qygsZscqczdjxxTable = qygsZscqczdjxxDiv.getElementById("table_zscq");
				if (qygsZscqczdjxxTable != null) {
					Elements qygsZscqczdjxxTrs = qygsZscqczdjxxTable.select("tr");
		...
		Object qygs_xzcfxx_object = resultHtmlMap.get("qygs_xzcfxx_page");
		if (qygs_xzcfxx_object != null) {
			String qygs_xzcfxx_page = (String)qygs_xzcfxx_object;
			Document qygsXzcfDoc = Jsoup.parse(qygs_xzcfxx_page);
			Element qygsXzcfXzcfxxDiv = qygsXzcfDoc.getElementById("sifapanding");
			if (qygsXzcfXzcfxxDiv != null) {
				Element qygsXzcfXzcfxxTable = qygsXzcfXzcfxxDiv.getElementById("table_qyxzcf");
				if (qygsXzcfXzcfxxTable != null) {
					Elements qygsXzcfXzcfxxTrs = qygsXzcfXzcfxxTable.select("tr");
				...
				if (isDebug) {
					qygsXzcfInfo.setHtml(qygsXzcfXzcfxxDiv.toString());
				}
			}
		}
		...
		Object qtbmgs_xzxkxx_object = resultHtmlMap.get("qtbmgs_xzxkxx_page");
		if (qtbmgs_xzxkxx_object != null) {
			String qtbmgs_xzxkxx_page = (String)qtbmgs_xzxkxx_object;
			Document qtbmgsXzxkxxDoc = Jsoup.parse(qtbmgs_xzxkxx_page);
			Element qtbmgsXzxkxxDiv = qtbmgsXzxkxxDoc.getElementById("xingzhengxuke");
			if (qtbmgsXzxkxxDiv != null) {
				Element qtbmgsXzxkxxTable = qtbmgsXzxkxxDiv.getElementById("table_xzxkother");
				if (qtbmgsXzxkxxTable != null) {
					Elements qtbmgsXzxkxxTrs = qtbmgsXzxkxxTable.select("tr");
					qtbmgsXzxkxxTrs.remove(0);
					qtbmgsXzxkxxTrs.remove(0);
					for (Element qtbmgsXzxkxxTr : qtbmgsXzxkxxTrs) {
						Elements qtbmgsXzxkxxTds = qtbmgsXzxkxxTr.select("td");
						if (qtbmgsXzxkxxTds.size() == 8) {
						...
						}else if (qtbmgsXzxkxxTds.size() == 9) {
							String xkwjNum = qtbmgsXzxkxxTds.get(1).text();
							String xkwjName = qtbmgsXzxkxxTds.get(2).text();
							String xzxkstartDate = qtbmgsXzxkxxTds.get(3).text();
							String xzxkendDate = qtbmgsXzxkxxTds.get(4).text();
		...
		Object qtbmgs_xzcfxx_object = resultHtmlMap.get("qtbmgs_xzcfxx_page");
		if (qtbmgs_xzcfxx_object != null) {
			String qtbmgs_xzcfxx_page = (String)qtbmgs_xzcfxx_object;
			Document qtbmgsXzcfxxDoc = Jsoup.parse(qtbmgs_xzcfxx_page);
			Element qtbmgsXzcfxxDiv = qtbmgsXzcfxxDoc.getElementById("xingzhengchufa");
			if (qtbmgsXzcfxxDiv != null) {
				Element qtbmgsXzcfxxTable = qtbmgsXzcfxxDiv.getElementById("table_qtxzcf");
				if (qtbmgsXzcfxxTable != null) {
					Elements qtbmgsXzcfxxTrs = qtbmgsXzcfxxTable.select("tr");
				...
				if (isDebug) {
					qtbmgsXzcfInfo.setHtml(qtbmgsXzcfxxDiv.toString());
				}
			}
		}
		...
		Object sfxzgs_gqdjxx_object = resultHtmlMap.get("sfxzgs_gqdjxx_page");
		if (sfxzgs_gqdjxx_object != null) {
			String sfxzgs_gqdjxx_page = (String)sfxzgs_gqdjxx_object;
			Document sfxzgsGqdjxxDoc = Jsoup.parse(sfxzgs_gqdjxx_page);
			Element sfxzgsDiv = sfxzgsGqdjxxDoc.getElementById("guquandongjie");
			if (sfxzgsDiv != null) {
				Element sfxzgsxxTable = sfxzgsDiv.getElementById("table_gqdj");
				if (sfxzgsxxTable != null) {
					Elements sfxzgsxxTrs = sfxzgsxxTable.select("tr");
		...
		Object sfxzgs_gdbgxx_object = resultHtmlMap.get("sfxzgs_gdbgxx_page");
		if (sfxzgs_gdbgxx_object != null) {
			String sfxzgs_gdbgxx_page = (String)sfxzgs_gdbgxx_object;
			Document sfxzgsGdbgxxDoc = Jsoup.parse(sfxzgs_gdbgxx_page);
			Element sfxzgsGdbgxxTable = sfxzgsGdbgxxDoc.getElementById("table_gdbg");
			if (sfxzgsGdbgxxTable != null) {
				Elements sfxzgsGdbgxxTrs = sfxzgsGdbgxxTable.select("tr");
				...
				if (isDebug) {
					sfxzgsGdbgInfo.setHtml(sfxzgsGdbgxxTable.toString());
				}
			}
		}
