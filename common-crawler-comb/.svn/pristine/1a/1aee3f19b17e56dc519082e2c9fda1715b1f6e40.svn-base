...
public class GsxtAnhuiParser extends AbstractGsxtParser {
	...
	public  AicFeedJson anhuiResultParser(String html, Boolean isDebug) {  
		...
		for (int i = 1; i < jbxx_ths.size(); i++) {
			...
			if (jbxx_ths.get(i).text().trim().contains("注册资本") ||jbxx_ths.get(i).text().trim().contains("注册资金")) {
				gsgsDjJbInfo.setRegCapital(jbxx_tds.get(i-1).text());// 注册资本
			}
			if (jbxx_ths.get(i).text().trim().contains("成立日期") || jbxx_ths.get(i).text().trim().contains("注册日期")) {
				gsgsDjJbInfo.setRegDateTime(jbxx_tds.get(i-1).text());// 成立日期
			}
			if (jbxx_ths.get(i).text().trim().contains("营业场所")
					|| jbxx_ths.get(i).text().trim().contains("住所") || jbxx_ths.get(i).text().trim().contains("经营场所")) {
