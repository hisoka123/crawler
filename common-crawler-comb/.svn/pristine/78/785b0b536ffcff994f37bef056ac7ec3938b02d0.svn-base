...
@Component
public class GsxtHainanParser {
	
	public AicFeedJson hainanResultParser(String resultHtmls, Boolean isDebug) {
		...
		//解析result
						...
						if (djxx_tables != null && !djxx_tables.isEmpty()) {
							...
							for (int i = 0; i < gsgsxxDjjbThs.size(); i++) {
								...
								case "经营场所":
									...
									break;
								case "组成形式":
									gsgsDjJbInfo.setFormType(gsgsxxDjjbTd);
									break;
								case "营业期限自":
				...
				if (qygsxx_object != null) {
					...
					System.out.println(qygsxx_qynb_detail_object);
					if (qygsxx_qynb_detail_object != null) {
						...
						if (qygsnbDiv != null) {
							...
							for (int j = 2; j < qygsnbTrs.size(); j++) {
								...
								if (qygsnbTds.size() == 3||qygsnbTds.size() == 4) {
								...
								for (Element qygsnbxxTable : qygsnbxxTables) {
										...
										for (int i = 0; i < qyjbxxThs.size(); i++) {
											...
											switch (qyjbxxTh) {
												...
												case "名称":
													...
													break;
												case "经营者姓名":
													qygsQynbJbInfo.setOperator(qyjbxxTd);
													break;
												case "联系电话":
													qygsQynbJbInfo.setTel(qyjbxxTd);
													break;
												...
												case "企业通信地址":
													...
													break;
												case "资金数额":
													qygsQynbJbInfo.setCapitalAmount(qyjbxxTd);
													break;
												case "企业电子邮箱":
													qygsQynbJbInfo.setEmail(qyjbxxTd);
													break;
									...
									} else if ("行政许可情况".equals(itemTitle)) {
										List<EntpubAnnreportAdmlicenseInfo> qygsQynbDwtgbzdbInfos = new ArrayList<EntpubAnnreportAdmlicenseInfo>();
										Elements dwdbxxTrs = qygsnbxxTable.select("tr");
										for (Element dwdbxxTr : dwdbxxTrs) {
										
												EntpubAnnreportAdmlicenseInfo qygsQynbDwtgbzdbInfo = new EntpubAnnreportAdmlicenseInfo();
												Elements dwdbxxTds = dwdbxxTr.select("td");
												if(dwdbxxTds.size()>0){																								
													String licenseName = dwdbxxTds.get(0).text();
													String licenseDate = dwdbxxTds.get(1).text();										
													qygsQynbDwtgbzdbInfo.setLicenseName(licenseName);
													qygsQynbDwtgbzdbInfo.setLicenseDate(licenseDate);								
													qygsQynbDwtgbzdbInfos.add(qygsQynbDwtgbzdbInfo);
													
												}
										
										}
										qygsQynbInfo.setAdmlicenseInfos(qygsQynbDwtgbzdbInfos);
									...
									} else if ("生产经营情况信息".equals(itemTitle)) {
										//企业年报信息--> 网站或网店信息
										List<EntpubAnnreportManageInfo> qygsQynbWzhwdInfos = new ArrayList<EntpubAnnreportManageInfo>();
										Elements wzwdxxTrs = qygsnbxxTable.select("tr");
										for (Element wzwdxxTr : wzwdxxTrs) {									
												Elements wzwdxxTds = wzwdxxTr.select("td");
												if (wzwdxxTds.size() == 3||wzwdxxTds.size() == 2) {
													String saleSum = wzwdxxTds.get(0).text();
													String salarySum = wzwdxxTds.get(1).text();
													
													EntpubAnnreportManageInfo qygsQynbWzhwdInfo = new EntpubAnnreportManageInfo();
													qygsQynbWzhwdInfo.setSalarySum(salarySum);
													qygsQynbWzhwdInfo.setSaleSum(saleSum);//营业额或营业收入										
													qygsQynbWzhwdInfos.add(qygsQynbWzhwdInfo);
												}
											
										}
										qygsQynbInfo.setManageInfos(qygsQynbWzhwdInfos);
