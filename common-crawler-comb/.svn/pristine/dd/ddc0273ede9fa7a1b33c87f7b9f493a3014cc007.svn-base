package com.crawler.zhengxin.login;
...
public class ZhengxinWebClient {
	
	private static final Logger log = LoggerFactory.getLogger(ZhengxinWebClient.class);
	
//	private static final String loginPage = "http://login.sina.com.cn/";
	
	/*rivate static final String checkPage = "http://weibo.com/";
	...
	private static ZhengxinWebClient instance;
	
	public static ZhengxinWebClient getInstance() {
		if (instance == null) {
			synchronized (ZhengxinWebClient.class) {
				if (instance == null) {
					instance = new ZhengxinWebClient();
				}
			}
		}
		return instance;
...
//		log.info("=============  用户登录   ============="+loginPage); 
	...
	public synchronized WebPageResponse getPageInlogin(String url,boolean jsEnable) throws Exception{
		...
		boolean islogin = ZhengxinWebClient.getInstance().isLogin(wr);
		if(islogin){
//			log.info("=============  已经登录    =============");
			return wr;
		}else{
