...
@Component
public class GsxtYunnanParser extends AbstractGsxtParser {

	private static final Logger log = LoggerFactory
			.getLogger(GsxtYunnanParser.class);

	public AicFeedJson yunnanParser(String html, boolean isDebug) {
...
//		changeInfos.add(aicpubRegChangeInfo);
		...
		if(resultHtmlMap.get("gsgsxx_zgbmxx")!=null){
			Elements eleszyryxx = Jsoup.parseBodyFragment(resultHtmlMap.get("gsgsxx_zgbmxx").toString()).select("table");
			if(eleszyryxx==null || eleszyryxx.isEmpty()) {
//				priMemberInfos=null;
			}else{
				Element ele_resultzyryxx=eleszyryxx.get(0);//获取第一个table
...
//					priMemberInfos=null;
					...
					if(tr_resultszyryxx.size()>2 && getElements(tr_resultszyryxx.get(0), "th")!=null && getElements(tr_resultszyryxx.get(0), "th").size()==1 && getElementText(getElements(tr_resultszyryxx.get(0), "th").get(0)).equals("主管部门（出资人）信息")){
						for(int i=2;i<tr_resultszyryxx.size();i++){
							if(getElements(tr_resultszyryxx.get(i), "td")!=null && getElements(tr_resultszyryxx.get(i), "td").size()==5){
		...
		archiveInfo.setMainDeptInfo(mainDeptInfo);
