...
public class ZjsfgkwLimitExitFunction {
	
	public String search(WebParam webParam) throws Exception {
		ChannelLogger LOGGER = ChannelLoggerFactory.getLogger(ZjsfgkwLimitExitFunction.class, webParam.getLogback());
		Map<String, String> resultMap = new LinkedHashMap<String, String>();
		Gson gson = new GsonBuilder().create();
		LOGGER.info("==================ZjsfgkwLimitExitFunction.search start!======================");
		WebClient webClient = WebCrawler.getInstance().getWebClient();

		HtmlPage searchPage = webClient.getPage(webParam.getUrl());
		
		if(searchPage.getElementsByIdAndOrName("txtExitReallyName").size()==0){
		...
		HtmlTextInput txtReallyName = (HtmlTextInput)(searchPage.getElementsByIdAndOrName("txtExitReallyName").get(0)); 
		txtReallyName.setText(params.get("txtreallyname"));
		HtmlTextInput txtCredentialsNumber = (HtmlTextInput)(searchPage.getElementsByIdAndOrName("txtExitCredentialsNumber").get(0)); 
		txtCredentialsNumber.setText(params.get("txtcredentialsnumber"));
		HtmlTextInput txtAH = (HtmlTextInput)(searchPage.getElementsByIdAndOrName("txtExitAH").get(0)); 
		txtAH.setText(params.get("txtah"));
		HtmlSelect drpZXFY = (HtmlSelect)(searchPage.getElementsByIdAndOrName("drpExitZXFY").get(0)); 
		...
		HtmlTextInput txtStartLARQ = (HtmlTextInput)(searchPage.getElementsByIdAndOrName("txtExitStartLARQ").get(0)); 
		txtStartLARQ.setText(params.get("txtstartlarq"));
		HtmlTextInput txtEndLARQ = (HtmlTextInput)(searchPage.getElementsByIdAndOrName("txtExitEndLARQ").get(0)); 
		txtEndLARQ.setText(params.get("txtendlarq"));

	
		//点击提交请求
		HtmlElement personalButton = (HtmlElement)(searchPage.getElementsByIdAndOrName("btnExitSearch").get(0)); 
