//从接口处获取详情
function iautosAjax_getDetailFromInterface(ctx,async,city,keyword,callFunName){
	    
	     var getDetailFromInterfaceResult='';
	     $.ajax({
	    	      url:ctx+"/api/iautos/getDataOnce",
	    	      type:"post",
	    	      async:async,
	    	      data:{
	    	    	   city:city,
...
//以post方式向详情页传递参数
function iautosAjax_windowOpenPostDetail(url,name,content){
...
//以post方式传递调试内容
function iautosAjax_windowOpenPostInterface(url,data,dataContent,error,errorContent){
...
function iautosAjax_checkIsExist(ctx,async,city,name,callFunName){

	     var getDetailFromDBResult='';
	     
	     $.ajax({
	    	 
	    	     url:ctx+"/modules/iautos/checkIsExist",
	    	     type:"post",
	    	     async:async,
	    	     data:{
	    	    	   city:city,
