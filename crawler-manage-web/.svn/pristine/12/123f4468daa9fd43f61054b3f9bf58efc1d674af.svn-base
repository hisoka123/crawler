...
$(function(){
    ...
    //生成人法网全部任务列表
    show_PeopleCourt_AllTask();

    //筛选div样式、动作
    peopleCourt_searchConStyle();
...
//查询第一页的内容
function show_PeopleCourt_AllTask(){

    var ctx='',loginName='',ownerTaskPeopleCourtPage='',taskList='';
    ...
    //taskList=getAllPeopleCourtTask(ctx,false,loginName);

    ownerTaskPeopleCourtPage=searchPeopleCourtTask(ctx,false,loginName,"searchType_all","searchStatus_all",null,pageSize,currentPage);
    taskList = ownerTaskPeopleCourtPage["content"];
    totalElements = ownerTaskPeopleCourtPage["totalElements"];
    ...
    $("#page").html("");
    //显示首页内容
    //peopleCourt_showPage(trs);
    peopleCourt_productList(taskList);

    //console.log("pageSize:"+ pageSize + "showPageNum: " +showPageNum +"=====totalElements=====" + totalElements);

    //调用commonSelf_pageStyle
    commonSelf_pageStyle(pageSize,showPageNum,totalElements,"isNull","page",getPeopleCourt__pageCallback);
...
//生成人法网任务列表
function peopleCourt_productList(taskList){
    ...
    $.each(taskList,function(index,task){
        ...
        trs +="<tr>"
            ...
            +"<td>"+task.keyWord+"</td>"
            +"<td>"+peopleCourt_taskState(task.state)+"</td>"
    ...
    $("table").on("click","a[id^='query_']",function(){
        ...
        peopleCourt_detail(tr,searchType,searchKeyword)
    });
...
//分页回调函数
function getPeopleCourt__pageCallback(){
    ...
    ownerTaskPeopleCourtPage=searchPeopleCourtTask(ctx,false,loginName,searchType,searchStatus,searchKeyword,pageSize,currentPage);
    currentPage_content = ownerTaskPeopleCourtPage["content"];

    //console.log("=====currentPage_content=====" + currentPage_content);

    //显示当前页
    peopleCourt_productList(currentPage_content);

}


//显示内容
/*function peopleCourt_showPage(content){
...
//任务状态
function peopleCourt_taskState(state){
...
//筛选　div样式
function peopleCourt_searchConStyle(){
    //状态筛选
    //==搜索类型选中==
    $("#chose_searchType").on("click","span[data-name]",function(){
        peopleCourtCommon_selected($(this),"chose_searchType","status_selectedStyle","status_unSelectedStyle");
    ...
    $("#chose_status").on("click","span[data-name]",function(){
        peopleCourtCommon_selected($(this),"chose_status","status_selectedStyle","status_unSelectedStyle");
    });
...
//单个选中
function peopleCourtCommon_selected(self,scope,selectedClass,unSelectedClass){
    var dataName='';
...
//搜索
function peopleCourt_search(){

    var ctx='',loginName='';
    var searchType='',searchStatus='',searchKeyword='';
    var ownerTaskPeopleCourtPage='',searchResult='';
    var noKeys='';
    ...
    currentPage = 1;
    ownerTaskPeopleCourtPage=searchPeopleCourtTask(ctx,false,loginName,searchType,searchStatus,searchKeyword,pageSize,currentPage);
    searchResult = ownerTaskPeopleCourtPage["content"];
    var totalElements = ownerTaskPeopleCourtPage["totalElements"];
    ...
    if(searchResult=="isError"){
        ...
        if(searchResult=='' ||searchResult.length==0){
            ...
            peopleCourt_productList(searchResult);
...
//显示详情
function peopleCourt_detail(tr,type,content){
	     ...
	     */
	     checkResult=renfawangAjax_checkIsExist(ctx,false,type,content);
