//从接口处获取详情
function sxjlcxptAjax_getDetailFromInterface(ctx,async,keyword,callFunName){
	    
	     var getDetailFromInterfaceResult='';
	     $.ajax({
	    	      url:ctx+"/api/sxjlcxpt/getDataOnce",
	    	      type:"post",
	    	      async:async,
	    	      data:{	    	    	
	    	    	   keyword:keyword,
...
//以post方式向详情页传递参数
function sxjlcxptAjax_windowOpenPostDetail(url,name,content){
...
//以post方式传递调试内容
function sxjlcxptAjax_windowOpenPostInterface(url,data,dataContent,error,errorContent){
...
function sxjlcxptAjax_checkIsExist(ctx,async,content,callFunName){
	
	     var getDetailFromDBResult='';
	     
	     $.ajax({
	    	 
	    	     url:ctx+"/modules/sxjlcxpt/checkIsExist",
	    	     type:"post",
	    	     async:async,
	    	     data:{
	    	    	   content:content,
