...
$(function(){
    ...
    //生成11315企业征信全部任务列表
    show_EnterpCredit_AllTask();

    //筛选div样式、动作
    enterpCredit_searchConStyle();
...
//查询第一页的内容
function show_EnterpCredit_AllTask(){

    var ctx='',loginName='',ownerTaskEnterpCreditPage='',taskList='';
    ...
    //taskList=getAllEnterpCreditTask(ctx,false,loginName);

    ownerTaskEnterpCreditPage=searchEnterpCreditTask(ctx,false,loginName,"searchStatus_all",null,pageSize,currentPage);
    taskList = ownerTaskEnterpCreditPage["content"];
    totalElements = ownerTaskEnterpCreditPage["totalElements"];
    ...
    $("#page").html("");
    //显示首页内容
    enterpCredit_productList(taskList);

    //console.log("pageSize:"+ pageSize + "showPageNum: " +showPageNum +"=====totalElements=====" + totalElements);

    //调用commonSelf_pageStyle
    commonSelf_pageStyle(pageSize,showPageNum,totalElements,"isNull","page",getEnterpCredit__pageCallback);

}


//生成11315企业征信任务列表
function enterpCredit_productList(taskList){
    ...
    $.each(taskList,function(index,task){
        ...
        trs +="<tr>"
            +"<td>"+lineIndex+"</td>"
            +"<td>"+task.keyWord+"</td>"
            +"<td>"+enterpCredit_taskState(task.state)+"</td>"
    ...
    $("table").on("click","a[id^='query_']",function(){
        ...
        enterpCreditTask_detail(keyword);
    });
...
//分页回调函数
function getEnterpCredit__pageCallback(){
    ...
    ownerTaskEnterpCreditPage=searchEnterpCreditTask(ctx,false,loginName,searchStatus,searchKeyword,pageSize,currentPage);
    currentPage_content = ownerTaskEnterpCreditPage["content"];

    //console.log("=====currentPage_content=====" + currentPage_content);

    //显示当前页
    enterpCredit_productList(currentPage_content);

}

//任务状态
function enterpCredit_taskState(state){
...
//筛选　div样式
function enterpCredit_searchConStyle(){
    //状态筛选
    //==状态选中==
    $("#chose_status").on("click","span[data-name]",function(){
        enterpCreditCommon_selected($(this),"chose_status","status_selectedStyle","status_unSelectedStyle");
    });
...
//单个选中
function enterpCreditCommon_selected(self,scope,selectedClass,unSelectedClass){
    var dataName='';
...
//搜索
function enterpCredit_search(){

    var ctx='',loginName='';
    var searchStatus='',searchKeyword='';
    var ownerTaskEnterpCreditPage='',searchResult='';
    var noKeys='';
    ...
    currentPage = 1;
    ownerTaskEnterpCreditPage=searchEnterpCreditTask(ctx,false,loginName,searchStatus,searchKeyword,pageSize,currentPage);
    searchResult = ownerTaskEnterpCreditPage["content"];
    var totalElements = ownerTaskEnterpCreditPage["totalElements"];
    ...
    if(searchResult=="isError"){
        ...
        if(searchResult=='' ||searchResult.isEmpty){
            ...
            enterpCredit_productList(searchResult);
...
//11315企业征信任务显示详情
function enterpCreditTask_detail(content){
    var ctx='',checkResult='',detailName="";

    ctx=$("#ctx").val();
    detailName="renfawangDetail";

    checkResult=renfawangAjax_checkIsExist(ctx,false,null,content);

    if(checkResult.state==1){
        renfawangAjax_windowOpenPostDetail(ctx+"/modules/zhengxin/zhengxinDetailByInterface",detailName,JSON.stringify(checkResult.ttSearchResult));
    }
