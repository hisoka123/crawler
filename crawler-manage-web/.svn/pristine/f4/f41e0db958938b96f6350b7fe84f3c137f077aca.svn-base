...
@Controller
@RequestMapping("/modules/ajaxZjsfgkwLimitExit")
public class AjaxZjsfgkwLimitExitController {

	private static final Logger log = LoggerFactory.getLogger(AjaxZjsfgkwLimitExitController.class);
	...
	/**
	   ...
	   */
	@RequestMapping(value="/getLimitExitResults",method=RequestMethod.GET)
	public @ResponseBody List<CreditJson> getSearchResults(@RequestParam("txtReallyName") String txtReallyName,@RequestParam("txtCredentialsNumber") String txtCredentialsNumber,
		 ...
		 try {
			  ...
			  Result<List<CreditJson>> result = zjsfgkwLimitExitService.searchLimitExit(url,true,"");          
