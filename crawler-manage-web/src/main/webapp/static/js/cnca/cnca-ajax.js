//从接口处获取详情
function cncaAjax_getDetailFromInterface(ctx,async,queryType,keyword,callFunName){
	    
	     var getDetailFromInterfaceResult='';
	     $.ajax({
	    	      url:ctx+"/api/cncaApiController/getDataOnce",
	    	      type:"post",
	    	      async:async,
	    	      data:{
	    	    	   keyword:keyword,
...
//以post方式向详情页传递参数
function cncaAjax_windowOpenPostDetail(url,name,content){
...
//以post方式传递调试内容
function cncaAjax_windowOpenPostInterface(url,data,dataContent,error,errorContent){
...
function cncaAjax_checkIsExist(ctx,async,queryType,content,callFunName){
	
	     var getDetailFromDBResult='';
	     
	     $.ajax({
	    	 
	    	     url:ctx+"/modules/cncaController/checkIsExist",
	    	     type:"post",
	    	     async:async,
	    	     data:{
	    	    	   content:content,
