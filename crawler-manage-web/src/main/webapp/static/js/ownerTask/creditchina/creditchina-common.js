...
$(function(){
    ...
    //生成信用中国全部任务列表
    show_CreditChina_AllTask();

    //筛选div样式、动作
    creditChina_searchConStyle();
...
function setSearchKeywordDisable(){
    ...
    if(searchType=="searchType_name" || searchType=="searchType_legalPerson"){
...
//查询第一页的内容
function show_CreditChina_AllTask(){

    var ctx='',loginName='',ownerTaskCreditChinaPage='',taskList='';
    ...
    //taskList=getAllCreditChinaTask(ctx,false,loginName);

    ownerTaskCreditChinaPage=searchCreditChinaTask(ctx,false,loginName,"searchType_all","searchStatus_all",null,pageSize,currentPage);
    taskList = ownerTaskCreditChinaPage["content"];
    totalElements = ownerTaskCreditChinaPage["totalElements"];
    ...
    $("#page").html("");
    //显示首页内容
    creditChina_productList(taskList);

    //console.log("pageSize:"+ pageSize + "showPageNum: " +showPageNum +"=====totalElements=====" + totalElements);

    //调用commonSelf_pageStyle
    commonSelf_pageStyle(pageSize,showPageNum,totalElements,"isNull","page",getCreditChina__pageCallback);
...
//生成信用中国任务列表
function creditChina_productList(taskList){

    var trs='',completeTaskDate='',btn='',remark='',objectType='';
    ...
    $.each(taskList,function(index,task){
        ...
        if(task.searchType==1) {
            objectType="自然人";
        }else if(task.searchType==2){
            objectType="法人";
        }
        ...
        trs +="<tr>"
            ...
            +"<td>"+objectType+"</td>"
            +"<td>"+task.keyWord+"</td>"
            +"<td>"+creditChina_taskState(task.state)+"</td>"
    ...
    $("table").on("click","a[id^='query_']",function(){
        ...
        creditChina_detail(type,keyword);
    });
...
//分页回调函数
function getCreditChina__pageCallback(){
    ...
    ownerTaskCreditChinaPage=searchCreditChinaTask(ctx,false,loginName,searchType,searchStatus,searchKeyword,pageSize,currentPage);
    currentPage_content = ownerTaskCreditChinaPage["content"];

    //console.log("=====currentPage_content=====" + currentPage_content);

    //显示当前页
    creditChina_productList(currentPage_content);

}

//任务状态
function creditChina_taskState(state){
...
//筛选　div样式
function creditChina_searchConStyle(){
    //状态筛选
    //==搜索类型选中==
    $("#chose_searchType").on("click","span[data-name]",function(){
        creditChinaCommon_selected($(this),"chose_searchType","status_selectedStyle","status_unSelectedStyle");
    ...
    $("#chose_status").on("click","span[data-name]",function(){
        creditChinaCommon_selected($(this),"chose_status","status_selectedStyle","status_unSelectedStyle");
    });
...
//单个选中
function creditChinaCommon_selected(self,scope,selectedClass,unSelectedClass){
    var dataName='';
...
//搜索
function creditChina_search(){

    var ctx='',loginName='';
    var searchType='',searchStatus='',searchKeyword='';
    var ownerTaskCreditChinaPage='',searchResult='';
    var noKeys='';
    ...
    currentPage = 1;
    ownerTaskCreditChinaPage=searchCreditChinaTask(ctx,false,loginName,searchType,searchStatus,searchKeyword,pageSize,currentPage);
    searchResult = ownerTaskCreditChinaPage["content"];
    var totalElements = ownerTaskCreditChinaPage["totalElements"];
    ...
    if(searchResult=="isError"){
        ...
        if(searchResult=='' ||searchResult.isEmpty){
            ...
            creditChina_productList(searchResult);
...
//显示详情
function creditChina_detail(type,content){

    var ctx='',checkResult='',detailName="",objectType;
    ...
    checkResult=creditchinaAjax_getDetailFromDB(ctx,false,content,objectType);

    if(checkResult.state==1){
        creditchinaAjax_windowOpenPostDetail(ctx+"/modules/creditchina/creditchinaDetail",detailName,JSON.stringify(checkResult.companyRecordResult));
    }
