//从接口处获取详情
function customsAjax_getDetailFromInterface(ctx,async,keyword,callFunName){
	    
	     var getDetailFromInterfaceResult='';
	     $.ajax({
	    	      url:ctx+"/api/customs/getDataOnce",
	    	      type:"post",
	    	      async:async,
	    	      data:{
	    	    	   keyword:keyword,
...
//以post方式向详情页传递参数
function customsAjax_windowOpenPostDetail(url,name,content){
...
//以post方式传递调试内容
function customsAjax_windowOpenPostInterface(url,data,dataContent,error,errorContent){
...
function customsAjax_checkIsExist(ctx,async,content,callFunName){
	
	     var getDetailFromDBResult='';
	     
	     $.ajax({
	    	 
	    	     url:ctx+"/modules/customs/checkIsExist",
	    	     type:"post",
	    	     async:async,
	    	     data:{
	    	    	   content:content,
