$(function(){
	
	//搜索框样式
	sxjlcxptCommon_searchBoxStyle();
	
	//搜索框聚焦
	$("#searchBox_big_content").focus(function(){
		$("#info >div").text("");
		$("#info").hide();
	})
	
	
	//判断环境
	var env=$("#env").val().trim();
	
	if(env=="prod"){

		  $("#chooseDataSource").remove();
		  $("#searchBox_big_btn").on("click",function(){
			  sxjlcxptCommon_detailFromDB();
		  })
		  
	}else{
		 $("#chooseDataSource").show();
		 sxjlcxptCommon_chooseSearchVersion()
		 
    }
	
})
//搜索框样式
function sxjlcxptCommon_searchBoxStyle(){
	
	
	
	//修正搜索框
    $("#searchBox_big >div[class='input-group'] >#searchBox_big_content").before("<div class='input-group-btn'>"
    		  +"<ul id='chooseType' class='dropdown-menu' style='font-size:14px;padding-left:5px;padding-right:5px'>"
              +"<li data-name='name' style='margin-top:10px;margin-bottom:10px;cursor:pointer'>失信记录机构</li>"
        +"</ul>"   
    		                                                                   
    		                                                                     +"</div>");
	
    $("#searchBox_big #chooseType").on("click","li",function(){
		
	       searchBoxPlaceholder("sxjlcxpt");
    })

$("#searchBox_big #chooseType li[data-name='name']").click();

	
	
}
//选择搜索版本
function sxjlcxptCommon_chooseSearchVersion(){
	
	    $("#searchBox_big_btn").on("click",function(){
	    	
	         var value='';
	         value=$("input[name='chooseDataSource']:checked").attr("value");
	
	         if(value=="interfaceVersion"){
	        	 sxjlcxptCommon_interfaceSearch(); 
	         }else if(value=="dbVersion"){
    	    	 sxjlcxptCommon_detailFromDB();
    	     }
	
       })
	
}
//接口搜索
function sxjlcxptCommon_interfaceSearch(){
	
	     var ctx='',type='',content='',searchResult='',queryType='';
	     
	     ctx=$("#ctx").val();
	    // type=$("#searchBox_big #type").text().trim();
	     content=$("#searchBox_big_content").val().trim();
	     
	     if(content==''){
	    	    alert("搜索内容不能为空");
	    	    return;
	     }
	     
	     
	     $("#info >div").text("努力查询，请稍候...");
		 $("#info >div").attr("style","color:red");
	     $("#info").show();
	     
	     setTimeout(function(){
	    	 
	    	    searchResult=sxjlcxptAjax_getDetailFromInterface(ctx,false,content);
     
	    	    sxjlcxptCommon_detailFromIN(ctx,searchResult);
	        	      
          },1000);
	
}
//接口处显示详情
function sxjlcxptCommon_detailFromIN(ctx,searchResult){
	
         var detailName='';
    
         detailName="sxjlcxptDetail";

         $("#info").hide();
 
         
         if(JSON.parse(searchResult).data!=null){
   	 
                   sxjlcxptAjax_windowOpenPostDetail(ctx+"/modules/sxjlcxpt/sxjlcxptDetailByInterface",detailName,JSON.stringify(JSON.parse(searchResult).data));
          
                  //调试时用
                 setTimeout(function(){
                	 sxjlcxptAjax_windowOpenPostInterface(ctx+"/modules/sxjlcxpt/sxjlcxptJson","data",JSON.stringify(JSON.parse(searchResult).data),"error","eNull");
                 },1000);
          
         }else{
   	     
   	           if(JSON.parse(searchResult).error.errorCode==7){//无关键词
   	 
                      alert(JSON.parse(searchResult).error.errorName);
          
               }else{
                      if(searchResult.indexOf("errorName")!=-1 && /[\u4e00-\u9fa5]/.test(searchResult)){
	                          alert("请求无效，可能原因：  "+JSON.parse(searchResult).error.errorName);
                      }else{
	                          alert("服务器繁忙，请重试！");
                      }
               }
         
               //调试时用
               setTimeout(function(){
            	   sxjlcxptAjax_windowOpenPostInterface(ctx+"/modules/sxjlcxpt/sxjlcxptJson","dNull","error",JSON.stringify(JSON.parse(searchResult).error));
               },1000);
          }
}
//从数据库获取详情、创建任务
function sxjlcxptCommon_detailFromDB(){
	
	     var ctx='',type='',content='',checkResult='',queryType=''
	
	   ctx=$("#ctx").val();
	   type=$("#searchBox_big #type").text().trim();
	   content=$("#searchBox_big_content").val().trim();
	   
	   if(content==''){
   	       alert("搜索内容不能为空");
   	       return;
       }
    	   	     
       detailName="sxjlcxptDetail";
	   loginName=$("#fat-menu").find("span[id='loginName']").text().trim();
	   
	   checkResult=sxjlcxptAjax_checkIsExist(ctx,false,content);
	   
	   if(checkResult.existCode==0){
		       
		       sxjlcxptCommon_confirmJoinModule(ctx,content,loginName,checkResult.existCode,checkResult.timetask_id);
	   }else{
		   
		       if(checkResult.state==7){  //existcode=1 && state=7
		    	  $("#info >div").text("没检索到关键字");
		    	  $("#info >div").attr("style","color:green");
                  $("#info").show();
		       }else if(checkResult.state==1){
		    	   sxjlcxptAjax_windowOpenPostDetail(ctx+"/modules/sxjlcxpt/sxjlcxptDetailByInterface",detailName,JSON.stringify(checkResult.ttSearchResult));
		       }else{
		    	   sxjlcxptCommon_confirmJoinModule(ctx,content,loginName,checkResult.existCode,checkResult.timetask_id);
		       }
		   
	   }
}
//创建新任务
function sxjlcxptCommon_confirmJoinModule(ctx,content,loginName,existCode,timetaskId){
	
	     var ownerTaskResult='';

	    ownerTaskResult=sxjlcxptAjax_joinTask(ctx,false,content,loginName,existCode,timetaskId);
 	         
 	     common_createNewTask("info",ownerTaskResult,content,"失信记录平台");
}

function sxjlcxptAjax_joinTask(ctx,async,content,loginName,existCode,timetaskId,callFunName){
    
    var joinTaskResult='';
    
    $.ajax({
    	
	          url:ctx+"/ownerTask/sxjlcxptJoinTask",
	          type:"post",
	          async:async,
	          data:{
	    	       content:content,
	    	       loginName:loginName,
	    	       existCode:existCode,
	    	       timetaskID:timetaskId,
	    	       date:new Date()
	         },
	         success:function(data){
	    	       if(!async){
	    		       joinTaskResult=data;
	    	       }
	         },
	         error:function(){
	    	     if(!async){
	    		    joinTaskResult="isError";
	    	     }
	         }
})

if(!async){
	    return joinTaskResult;
}

}