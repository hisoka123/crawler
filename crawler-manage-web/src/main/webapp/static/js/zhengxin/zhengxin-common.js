...
function renfawangCommon_searchBoxStyle(){
    ...
    $("#searchBox_big >div[class='input-group'] >#searchBox_big_content").before("<div class='input-group-btn'>"
    		                                                                           ...
    		                                                                           +"<ul id='chooseType' class='dropdown-menu' style='font-size:14px;padding-left:5px;padding-right:5px'>"
    		                                                                                 +"<li data-name='company' style='margin-top:10px;margin-bottom:10px;cursor:pointer'>公司名称</li>"
    		                                                                           +"</ul>"
    		                                                                     +"</div>");
	...
	$("#searchBox_big #chooseType li[data-name='company']").click();
...
function renfawangCommon_detailFromIN(ctx,searchResult){
         ...
         if(JSON.parse(searchResult).data!=null){
                   ...
                   renfawangAjax_windowOpenPostDetail(ctx+"/modules/zhengxin/zhengxinDetailByInterface",detailName,JSON.stringify(JSON.parse(searchResult).data));
          
                  //调试时用
                 setTimeout(function(){
                	 renfawangAjax_windowOpenPostInterface(ctx+"/modules/zhengxin/zhengxinJson","data",JSON.stringify(JSON.parse(searchResult).data),"error","eNull");
                 },1000);
...
function renfawangCommon_confirmJoinModule(ctx,type,content,loginName,existCode,timetaskId){
	     ...
	     ownerTaskResult=zhengxinAjax_joinTask(ctx,false,type,content,loginName,existCode,timetaskId);
 	         
 	     common_createNewTask("info",ownerTaskResult,type,content,"11315企业征信");
}


//保存新任务
function zhengxinAjax_joinTask(ctx,async,type,content,loginName,existCode,timetaskId,callFunName){
         
	     var joinTaskResult='';
         
	     $.ajax({
   	 
   	          url:ctx+"/modules/zhengxin/zhengxinJoinTask",
   	          type:"post",
   	          async:async,
   	          data:{
   	    	       type:type,
