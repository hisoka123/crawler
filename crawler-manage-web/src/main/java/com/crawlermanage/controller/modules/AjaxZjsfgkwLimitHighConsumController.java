...
@Controller
@RequestMapping("/modules/ajaxZjsfgkwLimitHighConsum")
public class AjaxZjsfgkwLimitHighConsumController {

	private static final Logger log = LoggerFactory.getLogger(AjaxZjsfgkwLimitHighConsumController.class);
	...
	/**
	   ...
	   */
	@RequestMapping(value="/getLimitHighConsumResults",method=RequestMethod.GET)
	public @ResponseBody List<CreditJson> getSearchResults(@RequestParam("txtReallyName") String txtReallyName,@RequestParam("txtCredentialsNumber") String txtCredentialsNumber,
		 ...
		 try {
			  ...
			  log.info("执行惩戒-限制高消费txtReallyName:{}",URLDecoder.decode(txtReallyName,"utf-8"));
			  ...
			  log.info("执行惩戒-限制高消费url:{}",url);
			  
			  Result<List<CreditJson>> result = zjsfgkwLimitHighConsumService.searchLimitHighConsum(url,true,"");          
		      ...
		      log.info("执行惩戒-限制高消费搜索结果:{}", creditJson);	
