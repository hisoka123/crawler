...
import com.crawler.zjsfgkw.htmlparser.LimitHighConsumFeedListParser;
...
@Component
public class ZjsfgkwLimitHighConsumService{
	
	@Autowired
	private CrawlerEngine crawlerEngine;
	
	@Autowired
	private LimitHighConsumFeedListParser limitHighConsumFeedListParser;
	  
	private static final Logger log = LoggerFactory.getLogger(ZjsfgkwLimitHighConsumService.class);
	
	
	//@Cacheable(value="dataCache", key="'ZjsfgkwLimitHighConsumService.searchLimitHighConsum_' + #url + '&isDebug=' + #isDebug", unless="#result==null || #isDebug==true")  
	public Result<List<CreditJson>> searchLimitHighConsum(String url, boolean isDebug, String logback) {
		
		log.info("url:{}",url);
		
		Result<List<CreditJson>> result = new Result<List<CreditJson>>();
		FunctionCallParam fcm = new FunctionCallParam();
		fcm.setFunction(FunctionDefine.ZJSFGKWLIMITHIGHCONSUM);
		...
		log.info("ZjsfgkwLimitHighConsumService.searchLimitHighConsum param:{}", param);
		...
		if (statusCodeDef!=null && StatusCodeDef.FREQUENCY_LIMITED.equals(statusCodeDef)) {
			...
			try {
				users = limitHighConsumFeedListParser.creditParser((String) resultHtmlMap.get("searchPageHtml"));
