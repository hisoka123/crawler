...
import com.crawler.zjsfgkw.htmlparser.LimitExitFeedListParser;
...
@Component
public class ZjsfgkwLimitExitService{
	
	@Autowired
	private CrawlerEngine crawlerEngine;
	
	@Autowired
	private LimitExitFeedListParser limitExitFeedListParser;
	  
	private static final Logger log = LoggerFactory.getLogger(ZjsfgkwLimitExitService.class);
	
	
	//@Cacheable(value="dataCache", key="'ZjsfgkwLimitExitService.searchLimitExit_' + #url + '&isDebug=' + #isDebug", unless="#result==null || #isDebug==true")  
	public Result<List<CreditJson>> searchLimitExit(String url, boolean isDebug, String logback) {
		
		log.info("url:{}",url);
		
		Result<List<CreditJson>> result = new Result<List<CreditJson>>();
		FunctionCallParam fcm = new FunctionCallParam();
		fcm.setFunction(FunctionDefine.ZJSFGKWLIMITEXIT);
		...
		log.info("ZjsfgkwLimitExitService.searchLimitExit param:{}", param);
		...
		if (statusCodeDef!=null && StatusCodeDef.FREQUENCY_LIMITED.equals(statusCodeDef)) {
			...
			try {
				users = limitExitFeedListParser.creditParser((String) resultHtmlMap.get("searchPageHtml"));
