...
import com.crawler.zjsfgkw.htmlparser.LimitBiddingFeedListParser;
...
@Component
public class ZjsfgkwLimitBiddingService{
	
	@Autowired
	private CrawlerEngine crawlerEngine;
	
	@Autowired
	private LimitBiddingFeedListParser limitBiddingFeedListParser;
	  
	private static final Logger log = LoggerFactory.getLogger(ZjsfgkwLimitBiddingService.class);
	
	
	//@Cacheable(value="dataCache", key="'ZjsfgkwLimitBiddingService.searchLimitBidding_' + #url + '&isDebug=' + #isDebug", unless="#result==null || #isDebug==true")  
	public Result<List<CreditJson>> searchLimitBidding(String url, boolean isDebug, String logback) {
		
		log.info("url:{}",url);
		
		Result<List<CreditJson>> result = new Result<List<CreditJson>>();
		FunctionCallParam fcm = new FunctionCallParam();
		fcm.setFunction(FunctionDefine.ZJSFGKWLIMITBIDDING);
		...
		log.info("ZjsfgkwLimitBiddingService.searchLimitBidding param:{}", param);
		...
		if (statusCodeDef!=null && StatusCodeDef.FREQUENCY_LIMITED.equals(statusCodeDef)) {
			...
			try {
				users = limitBiddingFeedListParser.creditParser((String) resultHtmlMap.get("searchPageHtml"));
