...
import com.crawler.zjsfgkw.htmlparser.CreditFeedListParser;
...
@Component
public class ZjsfgkwCreditService{
	
	@Autowired
	private CrawlerEngine crawlerEngine;
	
	@Autowired
	private CreditFeedListParser creditFeedListParser;
	  
	private static final Logger log = LoggerFactory.getLogger(ZjsfgkwCreditService.class);
	
	
	//@Cacheable(value="dataCache", key="'ZjsfgkwCreditService.searchCredit_' + #url + '&isDebug=' + #isDebug", unless="#result==null || #isDebug==true")  
	public Result<List<CreditJson>> searchCredit(String url, boolean isDebug, String logback) {
		
		log.info("url:{}",url);
		
		Result<List<CreditJson>> result = new Result<List<CreditJson>>();
		FunctionCallParam fcm = new FunctionCallParam();
		fcm.setFunction(FunctionDefine.ZJSFGKWCREDIT);
		...
		log.info("ZjsfgkwCreditService.searchCredit param:{}", param);
		...
		if (statusCodeDef!=null && StatusCodeDef.FREQUENCY_LIMITED.equals(statusCodeDef)) {
			...
			try {
				users = creditFeedListParser.creditParser((String) resultHtmlMap.get("searchPageHtml"));
